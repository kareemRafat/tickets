...
function handle_admin_remember_login($userId) {
    create_remember_token($userId, 'admin');
}

function process_admin_remember_login() {
    ...
    $db = getDBConnection();
    $stmt = $db->prepare("SELECT * FROM employees WHERE id = :id AND role = 'admin' AND status = 'active'");
